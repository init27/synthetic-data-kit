...
def test_rate_qa_pairs(patch_config):
    ...
    mock_client.chat_completion.return_value = json.dumps([
        {"question": "What is synthetic data?", "answer": "Synthetic data is artificially generated data.", "rating": 8},
        {"question": "Why use synthetic data?", "answer": "To protect privacy and create diverse training examples.", "rating": 6}
    ])
...
def test_process_document(patch_config):
    ...
    result = generator.process_document(
        document_text="This is a document to process.",
        num_pairs=2,
        verbose=False
    )
